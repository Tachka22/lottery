...
INSERT INTO tickets (draw_id, user_id, numbers, bonus, status, created_at) VALUES
    (1, 2, '5,12,33,41,50', NULL, 'WIN',  '2025-12-25 14:00:00'),  -- выигрышный
    (1, 2, '1,2,3,4,5',     NULL, 'LOSE', '2025-12-26 11:00:00');   -- проигрышный

-- Для тиража #2 (Весенний мега-тираж, ACTIVE) – билеты в статусе PENDING
INSERT INTO tickets (draw_id, user_id, numbers, bonus, status, created_at) VALUES
    (2, 2, '7,14,21,35,49,55', 3, 'PENDING', '2026-03-05 16:30:00'),
    (2, 2, '10,20,30,40,50,60', 7, 'PENDING', '2026-03-10 09:00:00');

-- Для тиража #3 (Летнее кено, DRAFT) – билетов пока нет

-- Для тиража #4 (Осенний кубок, CANCELLED) – билеты должны быть LOSE после отмены, но по логике можно вставить LOSE
INSERT INTO tickets (draw_id, user_id, numbers, bonus, status, created_at) VALUES
    (4, 2, '2,4,6,8,10', NULL, 'LOSE', '2026-04-10 16:00:00');

-- Для тиража #5 (Зимний тираж, FINISHED, без победителей)
INSERT INTO tickets (draw_id, user_id, numbers, bonus, status, created_at) VALUES
...
-- 6. История операций пользователя (user_actions)
